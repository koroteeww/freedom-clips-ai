...
4) Reuse existing generated images by default; only call gpt-image-1 if --image_mode generate
...
Example:
  python ai_viral_montage_clips.py \
    ...
    --outdir out \
    --n_clips 3 \\
    --reuse_images_from D:\\GLOBAL_Rayban_Meta\\_dur3
"""
...
def generate_image(client: OpenAI, model: str, prompt: str, out_path: Path, size: str = "1024x1536"):
    """
    Expensive path. This is used ONLY when --image_mode generate.
    Default mode is reuse_only, so no GPT image API calls happen by default.
    """
    log(f"[IMG API - PAID] {prompt}")
...
def extract_image_prompts_from_obj(obj: Any) -> List[str]:
    """
    Recursively scan previous scenario JSON files for hook.image_prompt / beat.image_prompt.
    Mostly for logs and future matching. Main cheap matching uses stable_hash(prompt)
    because earlier generated images are named hook_<hash>.png / beat_XX_<hash>.png.
    """
    prompts: List[str] = []
    if isinstance(obj, dict):
        for k, v in obj.items():
            if k == "image_prompt" and isinstance(v, str) and v.strip():
                prompts.append(v.strip())
            else:
                prompts.extend(extract_image_prompts_from_obj(v))
    elif isinstance(obj, list):
        for item in obj:
            prompts.extend(extract_image_prompts_from_obj(item))
    return prompts


def build_reuse_image_cache(scan_dir: Optional[Path]) -> Dict[str, Any]:
    """
    Scan a previous output folder, for example D:\GLOBAL_Rayban_Meta\_dur3,
    and find:
      - all generated images
      - hash-addressable images from filenames like hook_4d291cf26776.png
      - prompts from scenario JSON files

    No paid image generation is done here.
    """
    cache: Dict[str, Any] = {
    ...
    if not scan_dir:
        log("[IMG CACHE] No --reuse_images_from provided")
        return cache

    scan_dir = scan_dir.expanduser()
    if not scan_dir.exists():
        log(f"[IMG CACHE] Reuse folder does not exist: {scan_dir}")
        return cache

    image_exts = {".png", ".jpg", ".jpeg", ".webp"}
    hash_re = re.compile(r"([a-f0-9]{12})", re.I)

    images: List[Path] = []
    for path in scan_dir.rglob("*"):
        if path.is_file() and path.suffix.lower() in image_exts:
            images.append(path)
            m = hash_re.search(path.stem)
            if m:
                cache["by_hash"][m.group(1).lower()] = path

    prompts: List[str] = []
    for path in scan_dir.rglob("*.json"):
        # Skip huge/non-scenario JSON if needed, but these folders are usually small.
        try:
            obj = json.loads(path.read_text(encoding="utf-8"))
            prompts.extend(extract_image_prompts_from_obj(obj))
        except Exception:
            continue

    # Also map prompts to images when filename hash matches prompt stable hash.
    ...
    cache["all_images"] = sorted(images, key=lambda p: str(p).lower())
    cache["prompts"] = sorted(set(prompts))


    log(
        f"[IMG CACHE] Scanned {scan_dir}: "
        f"{len(cache['all_images'])} images, "
        f"{len(cache['by_hash'])} hash matches, "
        f"{len(cache['prompts'])} prompts"
    )

    return cache


def make_placeholder_image(prompt: str, out_path: Path):
    """
    Free fallback: create a simple local poster card with text.
    This avoids calling GPT image API when cache has no image.
    """
    require_pillow()
    out_path.parent.mkdir(parents=True, exist_ok=True)
    img = Image.new("RGB", (1080, 1920), (10, 12, 22))
    draw = ImageDraw.Draw(img)

    # Simple gradient-ish bands
    for y in range(1920):
        shade = int(18 + 45 * (y / 1920))
        draw.line([(0, y), (1080, y)], fill=(10, 12, min(70, shade)))

    # Decorative frame
    draw.rounded_rectangle([70, 120, 1010, 1800], radius=42, outline=(255, 255, 255), width=3)
    font_big = pick_font(64)
    font_small = pick_font(38)

    title = "DIGITAL FREEDOM"
    lines = wrap_text_to_width(prompt[:260], font_small, 850)

    tb = draw.textbbox((0, 0), title, font=font_big)
    draw.text(((1080 - (tb[2] - tb[0])) // 2, 220), title, font=font_big, fill=(255, 255, 255))

    y = 470
    for line in lines[:9]:
        bb = draw.textbbox((0, 0), line, font=font_small)
        draw.text(((1080 - (bb[2] - bb[0])) // 2, y), line, font=font_small, fill=(230, 235, 255))
        y += 58

    draw.text((110, 1660), "reused/local placeholder — no image API", font=pick_font(28), fill=(180, 190, 220))
    img.save(out_path)


def get_cached_or_create_image(
    prompt: str,
    out_path: Path,
    ...
    """
    image_mode:
      reuse_only  - default, never call image API; exact hash match -> copy; otherwise reuse deterministic fallback image; else local placeholder.
      placeholder - always create local placeholder cards, never call image API.
      generate    - exact reuse first, otherwise call GPT image API.
    """
    prompt = (prompt or "digital freedom").strip()
    h = stable_hash(prompt)

    if h in image_cache and image_cache[h].exists():
        return image_cache[h]

    out_path.parent.mkdir(parents=True, exist_ok=True)

    # 1) Exact hash match from previous generated images.
    by_hash: Dict[str, Path] = reuse_cache.get("by_hash", {})
    if h in by_hash and Path(by_hash[h]).exists():
        src = Path(by_hash[h])
        if not out_path.exists():
            shutil.copy2(src, out_path)
        image_cache[h] = out_path
        log(f"[IMG REUSE exact] {src} -> {out_path}")
        return out_path

    # 2) Placeholder mode: do not use real images at all.
    if image_mode == "placeholder":
        make_placeholder_image(prompt, out_path)
        image_cache[h] = out_path
        log(f"[IMG PLACEHOLDER] {out_path}")
        return out_path

    # 3) Reuse-only mode: deterministic fallback from image pool.
    all_images: List[Path] = reuse_cache.get("all_images", [])
    if image_mode == "reuse_only":
        if all_images:
            idx = int(h[:8], 16) % len(all_images)
            src = Path(all_images[idx])
            if not out_path.exists():
                shutil.copy2(src, out_path)
            image_cache[h] = out_path
            log(f"[IMG REUSE fallback] No exact hash for {h}; reused {src.name}")
            return out_path

        # No previous images at all: free local card.
        make_placeholder_image(prompt, out_path)
        image_cache[h] = out_path
        log(f"[IMG PLACEHOLDER no-cache] {out_path}")
        return out_path

    # 4) Paid generation only when explicitly requested.
    if image_mode == "generate":
        generate_image(client, image_model, prompt, out_path)
        image_cache[h] = out_path
        return out_path
...
def overlay_filter(base_filter: str, speed: float = 1.0) -> str:
    """Overlay PNG text and normalize video timestamps.

    Important: if we speed up audio with atempo, we must also speed up
    the video using setpts=PTS/speed. Otherwise each rendered part keeps
    the original video duration while audio is shorter, and concat may
    produce weird long timelines / silent tails.
    """
    speed = max(0.5, min(2.0, float(speed)))
    return (
        base_filter
        + ";[1:v]format=rgba[ov];"
        + f"[base][ov]overlay=0:0,setpts=PTS/{speed:.6f},fps=30,format=yuv420p[v]"
    )
...
def render_original_beat(input_mp4: Path, out_path: Path, start: float, duration: float, effect: str, speed: float, overlay_text: str):
    ...
    fc = overlay_filter(original_video_base_filter(effect), speed=speed)
...
def create_clip_assets(
    ...
    # Hook image: default is reuse_only, so this does NOT call paid image API.
    hook_hash = stable_hash(clip["hook"]["image_prompt"])
    hook_img = images_dir / f"hook_{hook_hash}.png"
    if not hook_img.exists():
        hook_img = get_cached_or_create_image(
            prompt=clip["hook"]["image_prompt"],
            out_path=hook_img,
    ...
    for idx, beat in enumerate(clip["beats"], start=1):
        ...
        if visual_source == "ai_image" and beat.get("image_prompt"):
            ...
            if not img_path.exists():
                img_path = get_cached_or_create_image(
                    prompt=prompt,
                    out_path=img_path,
                    image_cache=image_cache,
                    reuse_cache=reuse_cache,
...
def main():
    ...
    parser.add_argument("--gpt_model", default="gpt-4.1-mini")
    parser.add_argument("--image_model", default="gpt-image-1")
    parser.add_argument(
        "--image_mode",
        choices=["reuse_only", "placeholder", "generate"],
        default="reuse_only",
        help="reuse_only = never call GPT image API; reuse images from --reuse_images_from or make local placeholder. generate = paid image API fallback."
    )
    parser.add_argument(
        "--reuse_images_from",
        type=Path,
        default='D:\\GLOBAL_Rayban_Meta\\_dur3',
        help="Folder to scan for previous generated prompts/images, e.g. D:\\GLOBAL_Rayban_Meta\\_dur3. Defaults to outdir."
    )
    parser.add_argument("--min_unit_dur", type=float, default=3.0)
    parser.add_argument("--max_unit_dur", type=float, default=9.5)
    parser.add_argument("--max_ai_images_inside", type=int, default=2)
    ...
    log("[3/6] Building reusable image cache...")
    reuse_scan_dir = args.reuse_images_from or args.outdir
    reuse_cache = build_reuse_image_cache(reuse_scan_dir)
    if args.image_mode != "generate":
        log(f"[IMG MODE] {args.image_mode}: NO paid GPT image API calls will be made.")
    else:
        log("[IMG MODE] generate: paid GPT image API may be called when cache misses.")
